...
**스택**: Python · Google **Gemini**(`google-genai` function-calling) · `yfinance`·네이버 금융·`FinanceDataReader`(무료 데이터) · Streamlit · Hugging Face Spaces(Docker)
...
## 평가 결과 (정직성 하니스)
<!-- EVAL_RESULTS -->
`python eval/run_eval.py` 실행 결과로 자동 채워집니다.

## 실행 방법 (로컬)
```bash
pip install -r requirements.txt
# 무료 Gemini API 키: https://aistudio.google.com/apikey
cp .env.example .env        # Windows: copy .env.example .env  → GEMINI_API_KEY 입력
...
├─ agent.py          # Gemini function-calling 루프 + 정직성 시스템 지침 + 503/429 폴백
