...
## 평가 결과 (정직성 하니스)
`python eval/run_eval.py` 로 측정 — **GitHub Models(gpt-4o-mini) · 5케이스**.

| 지표 | 결과 | 의미 |
|---|---|---|
| 출처적중률(citation) | **100% (3/3)** | 수치형 답변 전부 도구 근거(EVIDENCE) 포함 |
| 수치정확도(±2%) | **100% (3/3)** | 삼성전자·애플·SK하이닉스 — 독립 조회값과 일치 |
| 거절률(존재X 종목) | **100% (2/2)** | 없는 종목엔 지어내지 않고 "찾지 못함"으로 거절 |

> 표본 5케이스의 소규모 측정이지만 **환각 없이 출처에 근거**함을 정량 확인. `eval/run_eval.py`로 재현 가능.
